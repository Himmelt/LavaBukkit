...
public class GameData {
    ...
    private static final BiConsumer<ResourceLocation, ForgeRegistry<?>> LOCK_VANILLA = (name, reg) -> reg.slaves
            .values().stream().filter(o -> o instanceof ILockableRegistry).forEach(o -> ((ILockableRegistry) o).lock());
    ...
    public static void init() {
        ...
        makeRegistry(BLOCKS, Block.class, MAX_BLOCK_ID, new ResourceLocation("air"))
                .addCallback(BlockCallbacks.INSTANCE).create();
        ...
        makeRegistry(RECIPES, IRecipe.class, MAX_RECIPE_ID).disableSaving().allowModification()
                .addCallback(RecipeCallbacks.INSTANCE).create();
        ...
        entityRegistry = (ForgeRegistry<EntityEntry>) makeRegistry(ENTITIES, EntityEntry.class, MAX_ENTITY_ID)
                .addCallback(EntityCallbacks.INSTANCE).create();
    }
    ...
    private static <T extends IForgeRegistryEntry<T>> RegistryBuilder<T> makeRegistry(ResourceLocation name,
        ...
        return new RegistryBuilder<T>().setName(name).setType(type).setMaxID(max)
                .addCallback(new NamespacedDefaultedWrapper.Factory<T>()).setDefaultKey(_default);
    }
    ...
    public static <K extends IForgeRegistryEntry<K>> K register_impl(K value) {
        ...
        Validate.notNull(value.getRegistryName(),
                String.format("Attempt to register object without having set a registry name %s (type %s)", value,
                        value.getClass().getName()));
        final IForgeRegistry<K> registry = GameRegistry.findRegistry(value.getRegistryType());
        Validate.notNull(registry,
                "Attempted to registry object without creating registry first: " + value.getRegistryType().getName());
    ...
    private static class BlockCallbacks
            implements IForgeRegistry.AddCallback<Block>, IForgeRegistry.ClearCallback<Block>,
            IForgeRegistry.CreateCallback<Block>, IForgeRegistry.DummyFactory<Block> {
        ...
        public void onAdd(IForgeRegistryInternal<Block> owner, RegistryManager stage, int id, Block block,
            ...
            @SuppressWarnings("unchecked")
            ClearableObjectIntIdentityMap<IBlockState> blockstateMap = owner.getSlaveMap(BLOCKSTATE_TO_ID,
            ...
            final ClearableObjectIntIdentityMap<IBlockState> idMap = new ClearableObjectIntIdentityMap<IBlockState>() {
                ...
                public int get(IBlockState key) {
                    ...
                    if (integer == null && key != null)
                        integer = this.identityMap
                                .get(key.getBlock().getStateFromMeta(key.getBlock().getMetaFromState(key)));
    ...
    private static class RecipeCallbacks
        ...
        public void onValidate(IForgeRegistryInternal<IRecipe> owner, RegistryManager stage, int id,
            ...
            if (!stage.getRegistry(Item.class).containsValue(item)) {
                throw new IllegalStateException(String.format("Recipe %s (%s) produces unregistered item %s (%s)", key,
                        obj, item.getRegistryName(), item));
            }
        }
    ...
    public static Multimap<ResourceLocation, ResourceLocation> injectSnapshot(
        ...
        snapshot.forEach((key, value) -> {
            ...
            loadPersistentDataToStagingRegistry(RegistryManager.ACTIVE, STAGING, remaps.get(key), missing.get(key), key,
                    value, clazz);
        });
    ...
    private static <T extends IForgeRegistryEntry<T>> void loadFrozenDataToStagingRegistry(RegistryManager STAGING,
        ...
        frozen.getKeys().stream().filter(key -> !newRegistry.containsKey(key))
    ...
    public static ResourceLocation checkPrefix(String name) {
        ...
        String prefix = mc == null || (mc instanceof InjectedModContainer
                && ((InjectedModContainer) mc).wrappedContainer instanceof FMLContainer) ? "minecraft"
                        : mc.getModId().toLowerCase(Locale.ROOT);
