...
public class EventBus implements IEventExceptionHandler {
    ...
    public void register(Object target) {
        ...
        Set<? extends Class<?>> supers = isStatic ? Sets.newHashSet((Class<?>) target)
                : TypeToken.of(target.getClass()).getTypes().rawTypes();
    ...
    private void register(Class<?> eventType, Object target, Method method, final ModContainer owner) {
        try {
            ...
            Unsafe theUnsafe = (Unsafe) un.get(null);
            Event event = (Event) theUnsafe.allocateInstance(eventType); //borderline cheating
            final ASMEventHandler asm = new ASMEventHandler(target, method, owner,
    ...
    public boolean post(Event event) {
        ...
        try {
            for (; index < listeners.length; index++) {
                Class<?> listenerClass = listeners[index].getClass();
