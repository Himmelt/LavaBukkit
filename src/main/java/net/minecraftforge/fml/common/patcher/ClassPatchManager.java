...
public class ClassPatchManager {
    ...
    public byte[] getPatchedResource(String name, String mappedName, DebuggableLaunchLoader loader) throws IOException {
    ...
    public byte[] applyPatch(String name, String mappedName, byte[] inputData) {
        ...
        if (DEBUG)
            FMLLog.log.debug("Runtime patching class {} (input size {}), found {} patch{}", mappedName,
        ...
        for (ClassPatch patch : list) {
            ...
            if (!patch.existsAtTarget && (inputData == null || inputData.length == 0)) {
                ...
                FMLLog.log.warn("Patcher expecting empty class data file for {}, but received non-empty",
                        patch.targetClassName);
            } else if (patch.existsAtTarget && (inputData == null || inputData.length == 0)) {
                FMLLog.log.fatal("Patcher expecting non-empty class data file for {}, but received empty.",
                        patch.targetClassName);
                ...
                if (patch.inputChecksum != inputChecksum) {
                    FMLLog.log.fatal(
                            ...
                            mappedName, name, Integer.toHexString(inputChecksum),
        ...
        if (dumpPatched) {
            try {
                ...
                FMLLog.log.error(FMLLog.log.getMessageFactory().newMessage("Failed to write {} to {}", mappedName,
    ...
    public void setup(Side side) {
        ...
        try {
            ...
            if (binpatchesCompressed == null) {
                if (!((Boolean) DebuggableLaunch.blackboard.get("fml.deobfuscatedEnvironment"))) {
    ...
    private ClassPatch readPatch(JarEntry patchEntry, JarInputStream jis) {
        ...
        try {
            ...
            FMLLog.log.warn(FMLLog.log.getMessageFactory().newMessage("Unable to read binpatch file {} - ignoring",
                    patchEntry.getName()), e);
            return null;
        }
