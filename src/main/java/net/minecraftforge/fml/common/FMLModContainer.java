...
public class FMLModContainer implements ModContainer {
    ...
    public FMLModContainer(String className, ModCandidate container, Map<String, Object> modDescriptor) {
        ...
        if (Strings.isNullOrEmpty(languageAdapterType)) {
            ...
            FMLLog.log.trace("Using custom language adapter {} for {} (modid: {})", languageAdapterType, this.className,
                    getModId());
        }
        sanityCheckModId();
    }
    ...
    private ILanguageAdapter getLanguageAdapter() {
        if (languageAdapter == null) {
            try {
                languageAdapter = (ILanguageAdapter) Class.forName((String) descriptor.get("modLanguageAdapter"), true,
                        Loader.instance().getModClassLoader()).newInstance();
            } catch (Exception ex) {
                FMLLog.log.error("Error constructing custom mod language adapter referenced by {} (modid: {})",
                        getModId(), ex);
    ...
    public void bindMetadata(MetadataCollection mc) {
        ...
        if (overridesMetadata || !modMetadata.useDependencyInformation) {
            ...
            FMLLog.log.trace("Parsed dependency info for {}: Requirements: {} After:{} Before:{}", getModId(),
                    info.requirements, info.dependencies, info.dependants);
        } else {
            FMLLog.log.trace("Using mcmod dependency info for {}: {} {} {}", getModId(), modMetadata.requiredMods,
        ...
        if (Strings.isNullOrEmpty(internalVersion)) {
            ...
            if (versionProps != null) {
                ...
                FMLLog.log.debug("Found version {} for mod {} in version.properties, using", internalVersion,
                        getModId());
            }
                ...
                if (a.annotationType().equals(Mod.EventHandler.class)) {
                    ...
                    if (Modifier.isStatic(m.getModifiers()) && m.getParameterTypes().length == 0
                        ...
                        FMLLog.log.error(
                                ...
                                m.getName(), Arrays.asList(m.getParameterTypes()), getModId());
    ...
    private void parseSimpleFieldAnnotation(SetMultimap<String, ASMData> annotations, String annotationClassName,
        ...
        for (ASMData targets : annotations.get(annotationClassName)) {
            ...
            if (Strings.isNullOrEmpty(owner)) {
                ...
                if (Strings.isNullOrEmpty(owner)) {
                    FMLLog.bigWarning("Could not determine owning mod for @{} on {} for mod {}", annotationClassName,
                            targets.getClassName(), this.getModId());
                    continue;
                }
            }
            if (!this.getModId().equals(owner)) {
                FMLLog.log.debug("Skipping @{} injection for {}.{} since it is not for mod {}", annotationClassName,
                        targets.getClassName(), targets.getObjectName(), this.getModId());
                continue;
            }
            Field f = null;
            ...
            if (mc != null) {
                try {
                    ...
                    FMLLog.log.warn("Attempting to load @{} in class {} for {} and failing", annotationName,
                            targets.getClassName(), mc.getModId(), e);
                }
            }
            if (f != null) {
                Object target = null;
    ...
    public void constructMod(FMLConstructionEvent event) {
        ...
        try {
            ...
            FormattedMessage message = new FormattedMessage("{} Failed to add file to classloader: {}", getModId(),
                    source);
        ...
        NetworkRegistry.INSTANCE.register(this, clazz,
                (String) (descriptor.getOrDefault("acceptableRemoteVersions", null)), event.getASMHarvestedData());
    ...
    public Class<?> getCustomResourcePackClass() {
        try {
            return getSource().isDirectory()
                    ? Class.forName("net.minecraftforge.fml.client.FMLFolderResourcePack", true,
                            Thread.currentThread().getContextClassLoader())
                    : Class.forName("net.minecraftforge.fml.client.FMLFileResourcePack", true,
                            Thread.currentThread().getContextClassLoader());
        } catch (ClassNotFoundException e) {
            return null;
        }
