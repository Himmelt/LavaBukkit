...
public class FMLClientHandler implements IFMLSidedHandler {
    ...
    public void beginMinecraftLoading(Minecraft minecraft, List<IResourcePack> resourcePackList,
        ...
        Map<String, Map<String, String>> sharedModList = (Map<String, Map<String, String>>) DebuggableLaunch.blackboard
                .get("modList");
        if (sharedModList == null) {
            sharedModList = Maps.newHashMap();
            DebuggableLaunch.blackboard.put("modList", sharedModList);
        }
    ...
    private void detectOptifine() {
        try {
            ...
            try (InputStream optifineModInfoInputStream = getClass().getResourceAsStream("optifinemod.info")) {
                ...
                FMLLog.log.info("Forge Mod Loader has detected optifine {}, enabling compatibility features",
    ...
    public void finishMinecraftLoading() {
        ...
        for (ModContainer mc : Loader.instance().getActiveModList()) {
            ...
            try {
                ...
                FMLLog.log.error("A critical error occurred instantiating the gui factory for mod {}", mc.getModId(),
                        e);
            }
        }
        loading = false;
    ...
    public void extendModList() {
        @SuppressWarnings("unchecked")
        Map<String, Map<String, String>> modList = (Map<String, Map<String, String>>) DebuggableLaunch.blackboard
                .get("modList");
        if (modList != null) {
    ...
    public void addModAsResource(ModContainer container) {
        ...
        if (resourcePackType != null) {
            try {
                ...
                FMLLog.log.error("The container {} (type {}) returned an invalid class for its resource pack.",
                ...
                FormattedMessage message = new FormattedMessage(
                        ...
                        container.getName(), container.getModId());
    ...
    public void bindServerListData(ServerData data, ServerStatusResponse originalResponse) {
        if (extraServerListData.containsKey(originalResponse)) {
            ...
            serverDataTag.put(data,
                    new ExtendedServerListData(type, modRejections == null, modListMap, !moddedClientAllowed));
        } else {
            ...
            serverDataTag.put(data,
                    new ExtendedServerListData("VANILLA", false, ImmutableMap.of(), !moddedClientAllowed));
        }
    ...
    public void fireNetRegistrationEvent(EventBus bus, NetworkManager manager, Set<String> channelSet, String channel,
            Side side) {
        if (side == Side.CLIENT) {
            bus.post(new FMLNetworkEvent.CustomPacketRegistrationEvent<NetHandlerPlayClient>(manager, channelSet,
                    channel, side, NetHandlerPlayClient.class));
        } else {
            bus.post(new FMLNetworkEvent.CustomPacketRegistrationEvent<NetHandlerPlayServer>(manager, channelSet,
                    channel, side, NetHandlerPlayServer.class));
        }
    }
    ...
    public IThreadListener getWorldThread(INetHandler net) {
        if (net instanceof INetHandlerPlayClient || net instanceof INetHandlerLoginClient
                || net instanceof INetHandlerStatusClient)
            return getClient();
        if (net instanceof INetHandlerHandshakeServer || net instanceof INetHandlerLoginServer
                || net instanceof INetHandlerPlayServer || net instanceof INetHandlerStatusServer)
            return getServer();
    ...
    public void trackBrokenTexture(ResourceLocation resourceLocation, String error) {
        ...
        if (badType == null) {
            ...
            brokenTextures.put(resourceLocation.getResourceDomain(), MoreObjects.firstNonNull(error, "Unknown error"),
                    badType);
        }
