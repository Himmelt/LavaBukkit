...
public class CoreModManager {
    ...
    private static class FMLPluginWrapper implements ITweakerDebuggable {
        ...
        public void injectIntoClassLoader(DebuggableLaunchLoader classLoader) {
            FMLLog.log.debug("Injecting coremod {} \\{{}\\} class transformers", name,
    ...
    public static void handleLaunch(File mcDir, DebuggableLaunchLoader classLoader, FMLTweaker tweaker) {
        ...
        try {
            ...
            throw new RuntimeException(
                    "The patch transformer failed to load! This is critical, loading cannot continue!", e);
        }
    ...
    private static void findDerpMods(DebuggableLaunchLoader classLoader, File modDir, File modDirVer) {
        ...
        if (derpdirlist != null && derpdirlist.length > 0) {
            ...
            for (File f : derpdirlist) {
                FMLLog.log.fatal("Directory {} contains {}", f.getName(),
            ...
            try {
                ...
                File crashreportfile = new File(new File(modDir.getParentFile(), "crash-reports"),
                ...
                FMLLog.log.fatal("#@!@# FML has crashed the game deliberately. Crash report saved to: #@!@# {}",
    ...
    private static void discoverCoreMods(File mcDir, DebuggableLaunchLoader classLoader) {
        ...
        for (File coreMod : file_canidates) {
            ...
            try {
                ...
                if (modSide != null && !"BOTH".equals(modSide) && !FMLLaunchHandler.side().name().equals(modSide)) {
                    FMLLog.log.debug("Mod {} has ModSide meta-inf value {}, and we're {} It will be ignored",
                ...
                if (!modTypes.contains("FML")) {
                    FMLLog.log.debug("Adding {} to the list of things to skip. It is not an FML mod, it has types {}",
    ...
    private static void handleCascadingTweak(File coreMod, JarFile jar, String cascadedTweaker,
            DebuggableLaunchLoader classLoader, Integer sortingOrder) {
        try {
    ...
    private static FMLPluginWrapper loadCoreMod(DebuggableLaunchLoader classLoader, String coreModClass,
            File location) {
        ...
        try {
            ...
            if (accessTransformerClass != null) {
                FMLLog.log.debug("Added access transformer class {} to enqueued access transformers",
    ...
    public static void injectTransformers(DebuggableLaunchLoader classLoader) {

        DebuggableLaunch.blackboard.put("fml.deobfuscatedEnvironment", deobfuscatedEnvironment);
    ...
    public static void injectCoreModTweaks(FMLInjectionAndSortingTweaker fmlInjectionAndSortingTweaker) {
        ...
        List<ITweakerDebuggable> tweakers = (List<ITweakerDebuggable>) DebuggableLaunch.blackboard.get("Tweaks");
    ...
    public static void sortTweakList() {
        ...
        ITweakerDebuggable[] toSort = tweakers.toArray(new ITweakerDebuggable[tweakers.size()]);
        ToIntFunction<ITweakerDebuggable> getOrder = o -> o instanceof FMLInjectionAndSortingTweaker ? Integer.MIN_VALUE
                : o instanceof FMLPluginWrapper ? ((FMLPluginWrapper) o).sortIndex
                        : tweakSorting.getOrDefault(o.getClass().getName(), 0);
        Arrays.sort(toSort,
